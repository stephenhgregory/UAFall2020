 ...
 * CWID: 11812663
 * 
 * This program acts as a simple bash-like shell.
 * 
 * The user may enter such commands as `ls`, `clear`, and other UNIX commands.
 * When the user appends the command with an ampersand symbol (&), the parent process
 * (the shell program) will not wait for the child process (The process created by the 
 * command). Otherwise, the parent process WILL wait for the child process, suspending
 * execution until that child process has exited.
 * 
*/
